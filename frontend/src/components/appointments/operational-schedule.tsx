...
export function ScheduleOperations() {
  ...
  return <section className="mt-8" aria-labelledby="schedule-heading">
    ...
    {selected && action === "audit" && <LifecyclePanel title="Appointment audit timeline" onClose={() => setAction(null)}><ol className="grid gap-3">{audit.data?.results.map((event) => <li key={event.id} className="rounded-xl border p-3"><strong>{event.event} Â· {event.outcome}</strong><p className="text-sm text-slate-600">{new Date(event.created_at).toLocaleString()} Â· {event.actor_name}</p>{event.rejection_code && <p className="text-sm">Policy result: {event.rejection_code}</p>}{event.reason_category && <p className="text-sm">Category: {event.reason_category}</p>}{event.override_used && <p className="text-sm font-semibold">Owner override audited</p>}</li>)}</ol></LifecyclePanel>}
  </section>;
}
